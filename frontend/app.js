...
function exportFilename(session, ext) {
  ...
  return `co-inventor-${words}-round${round}.${ext}`;
}
...
function exportMarkdown() {
  ...
  lines.push(`*Co-Inventor · ${date} · Round ${round}*`);
  lines.push('');
  lines.push('---');
  lines.push('');
  ...
  lines.push(`*Session ${session.id} · Generated by Co-Inventor using the Co-Scientist architecture (Google DeepMind, 2025)*`);
